   ...
   public function index(Request $request)
{
    ...
    // 3. Urutan & Paginasi (PENTING: Gunakan paginate, bukan get)
    // Oldest() sesuai permintaan Anda (terlama ke terbaru)
    $surat_terbaru = $query->oldest()->paginate(10);

    // Statistik tetap diambil secara terpisah
    $stats = [
        'diajukan' => Surat::where('status', 'Diajukan')->count(),
        'diproses' => Surat::where('status', 'Diproses')->count(),
        'ditolak'  => Surat::where('status', 'Ditolak')->count(),
    ...
    {
        // Mengambil detail surat, data warga, dan list pegawai untuk TTD
    ...
    /**
     * Memproses surat (Update status jadi 'Diproses' & Input No Surat)
     */
    public function proses(Request $request, $id)
    {
        ...
        $surat->update([
            'status' => 'Diproses', // Sesuai enum di DB Anda
