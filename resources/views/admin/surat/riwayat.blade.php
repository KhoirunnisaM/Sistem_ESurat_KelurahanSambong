...
<div id="realtime-container" class="container-fluid px-2 px-md-3">
    {{-- 1. HEADER HALAMAN --}}
    <div class="mb-4">
        <h4 class="fw-bold text-dark responsive-title">Riwayat Pengajuan Surat</h4>
        <p class="text-muted small responsive-sub">Arsip permohonan dengan status <b>SELESAI</b>, <b>DITOLAK</b>, dan <b>DIBATALKAN</b>.</p>
    </div>
    ...
    <div class="card card-custom bg-white border-0 shadow-sm overflow-hidden">
        <div class="card-body p-3 p-md-4">
            <div class="d-flex flex-column d-md-flex flex-md-row justify-content-between align-items-md-center mb-4">
                <div>
                    <h5 class="fw-bold mb-1 responsive-h6">Arsip Surat</h5>
                    <p class="text-muted small mb-0 responsive-text">Menampilkan data yang telah diproses final.</p>
                </div>
                
                <div class="mt-3 mt-md-0">
                    <form action="{{ route('admin.surat.riwayat') }}" method="GET" class="d-flex flex-wrap flex-md-nowrap gap-2">
                        ...
                        @if(request('search') || request('tanggal') || (request('status') && request('status') != 'semua'))
                            <a href="{{ route('admin.surat.riwayat') }}" class="btn btn-sm btn-outline-secondary rounded-pill d-flex align-items-center">Reset</a>
                        @endif
                    </form>
                </div>
            </div>
                        ...
                        <tr style="font-size: 0.85rem;">
                            ...
                            <td>
                                ...
                                @elseif($s->status == 'Ditolak' && $s->alasan_ditolak)
                                    <small class="text-danger d-block fst-italic text-truncate" style="font-size: 0.65rem; max-width: 180px;">* {{ $s->alasan_ditolak }}</small>
                                @elseif($s->status == 'Dibatalkan')
                                    <small class="text-muted fst-italic" style="font-size: 0.65rem;">* Dibatalkan oleh warga</small>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="text-primary fw-medium" style="font-size: 0.75rem;">{{ $s->updated_at->format('d/m/Y') }}</div>
                                <small class="text-muted" style="font-size: 0.65rem;">{{ $s->updated_at->format('H:i') }} WIB</small>
                            </td>
                            <td>
                                @php
                            ...
                            <td class="text-center">
                                <a href="{{ route('admin.surat.show', $s->id) }}?from=riwayat" class="btn btn-sm btn-outline-dark rounded-pill px-2 py-0" style="font-size: 0.7rem;">
                                    Detail
                                </a>
                            </td>
                        </tr>
