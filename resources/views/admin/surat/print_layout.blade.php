@php
    // Logika Pembersihan Judul Surat (Hapus kata "UMUM")
    ...
    // Jika bukan Lurah, tambahkan a.n LURAH SAMBONG
    ...
    // Fungsi bantu kapitalisasi sesuai KBBI
...
<head>
    ...
    <style>
        ...
        body { 
            ...
            font-size: 12pt; line-height: 1.5; color: black; margin: 0; padding: 0;
        }
        .page { 
            background: white; width: 210mm; min-height: 297mm; 
            padding: 20mm 20mm 20mm 30mm; margin: auto; box-sizing: border-box;
        }
        ...
        /* Class Khusus Nama Pejabat (Atas/Isi) - TANPA Garis Bawah */
        .nama-pejabat-isi { font-weight: bold; text-transform: uppercase; text-decoration: none; }
        
        /* Class Khusus Nama Pejabat (Bawah/TTD) - DENGAN Garis Bawah */
    ...
    <div class="page">
        ...
        <div class="isi">
            ...
            @if(str_contains(strtoupper($surat->jenis_surat), 'DOMISILI USAHA'))
                <table class="data-table">
                    <tr><td width="30%">a. Nama Pemilik</td><td width="2%">:</td><td class="nama-pejabat-isi">{{ $surat->warga->nama_lengkap }}</td></tr>
                    <tr><td>b. Tempat/Tgl Lahir</td><td>:</td><td>{{ ucwords(strtolower($surat->warga->tempat_lahir)) }}, {{ $surat->warga->tanggal_lahir }}</td></tr>
                    <tr><td>c. Alamat Pemilik</td><td>:</td><td>{{ ucwords(strtolower($surat->warga->alamat_lengkap)) }} RT {{ $surat->warga->rt }} RW {{ $surat->warga->rw }} Kel. Sambong</td></tr>
                    <tr><td>d. Nama Usaha</td><td>:</td><td style="font-weight:bold;">{{ strtoupper($surat->nama_usaha ?? '................') }}</td></tr>
                    <tr><td>e. Jenis Usaha</td><td>:</td><td>{{ formatKbbi($surat->jenis_usaha ?? $surat->keperluan) }}</td></tr>
                    <tr><td>f. Alamat Usaha</td><td>:</td><td>{{ formatKbbi($surat->alamat_usaha ?? 'Kelurahan Sambong, Batang') }}</td></tr>
                </table>
            @else
                <table class="data-table">
        ...
        <div class="ttd-container">
            <p>Batang, {{ \Carbon\Carbon::parse($surat->tanggal_surat_ttd)->translatedFormat('d F Y') }}</p>
